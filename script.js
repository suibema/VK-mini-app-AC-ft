...
const USE_SQL_FUNCTIONS = false; // NocoDB does not support custom SQL functions

// NocoDB Configuration
const NOCODB_BASE_URL = "https://ndb.fut.ru"; // Replace with your NocoDB instance URL
const NOCODB_API_TOKEN = "crDte8gB-CSZzNujzSsy9obQRqZYkY3SNp8wre88"; // Replace with your NocoDB API token
...
async function checkUserExists() {
  if (!window.vkUserId) {
    setMsg("Не удалось определить VK ID.", "err");
    return false;
  }

  try {
    const data = await nocodbRequest("GET", `/${TBL_REG}/records/count?where=(${COL_REG_TGID},eq,${window.vkUserId})`);
    if (data.count === 0) {
...
async function loadCurrentBooking() {
  if (!window.vkUserId) return { has: false };

  try {
    const data = await nocodbRequest("GET", `/${TBL_REG}/records?where=(${COL_REG_TGID},eq,${window.vkUserId})&limit=1`);
